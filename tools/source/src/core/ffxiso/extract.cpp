...
bool ffxiso::extract(std::filesystem::path iso_path, std::filesystem::path output_dir)
{
    ...
    for (int i = 0; i < CDROM_MDG_SIZE; i++)
    {
        ...
        if (binfile::has_bytes(file_bytes, 5, {0x01, 0x01, 0xc0}) || binfile::has_bytes(file_bytes, 5, {0x01, 0x01, 0xb0}))
            ...
            ext = "ftcx";
        else if (binfile::has_bytes(file_bytes, 0x5, {0x88, 0x00, 0x01, 0x01, 0xa0, 0x07}) || binfile::has_bytes(file_bytes, 0x5, {0x90, 0x00, 0x01, 0x01, 0xc0, 0x07}))
            ext = "ffa";
        else if (binfile::has_bytes(file_bytes, 0x5, {0x88, 0x00, 0x01, 0x0b, 0x80, 0x03}) || binfile::has_bytes(file_bytes, 0x5, {0x90, 0x00, 0x01, 0x0b, 0x80, 0x03}))
            ext = "ffm";
        else if (binfile::has_bytes(file_bytes, 0, {0x08, 0x10, 0x13, 0x16, 0x1a, 0x1b, 0x1d, 0x22, 0x10, 0x10, 0x16, 0x18, 0x1b, 0x1d, 0x22, 0x25}))
            ext = "mov";
        else
            ext = "bin";
